...
<?php
$buyerDB = new CreateDb("Organic_India", "users");
$buyerList = $buyerDB->getData();

while($row = mysqli_fetch_assoc($buyerList)){
    // echo $row['name']."<br>";
    $person = "
    <tr>
    <form action=\"buyerList.php?action=remove&id=".$row['id']."\" method=\"post\">
        <td>".$row['name']."</td>
        <td>".$row['email']."</td>
